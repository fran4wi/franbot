...
class Google_Container:
    ...
    def __init__(self, credentials_dict: dict, sheet_infos: list):
        """
    ...
    def _check_for_service_account(self, args):
        if self.service_account == None:
            raise Exception(
                f"It looks like our Google Container has not been Initialized!  {args}"
            )
    ...
    def LOG_ERROR(self, row: list):
        ...
        try:
            ...
            raise Exception(
                f"ERRORS google sheet does not seem to exist! {__file__}:ERRORS()"
            )
    ...
    def fran(self, row: list = []):
        ...
        try:
            ...
            raise Exception(
                f"ERRORS google sheet does not seem to exist! {__file__}:FRAN_COMMAND()"
            )
